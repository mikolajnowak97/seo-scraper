...
class GoogleMaps:
    ...
    def exists(self):
        ...
        cur.execute(
            f"SELECT EXISTS(SELECT 1 FROM public.googlemaps WHERE search_phrase='{self.search_phrase}' AND displayname='{self.displayname}' AND search_order={self.search_order})"
        )
        record = cur.fetchone()
        cur.close()
